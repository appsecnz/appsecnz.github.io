---
title: Classroom Training - Wifi - Novice to Professional
description: Pre-Conference Training Class, 2 September - OWASP New Zealand Day 2026

layout: conference/full-width
dataDir: conference-2026
sponsorsEventName: OWASP New Zealand Day 2026
---

[![Web Banner](/assets/images/2026_Banner_Graphic.jpg)](/conference/)   
[Back to Pre-Conference Training Page](training.md)
{: style="text-align: right; font-size: small;" }   
[Back to Conference Home Page](index.md)
{: style="text-align: right; font-size: small;" }   

# Wifi - Novice to Professional

## Half-Day Interactive (Classroom) Training - OWASP New Zealand Day 2026

## Abstract

This live hands-on training equips you with the knowledge and expertise to perform penetration tests on modern day WiFi networks. The included lab environment contains both personal and enterprise networks allowing for practice even with the latest WPA standards preparing you for the real world!

## Course Details

**Dates:** Wednesday, 2 September 2026

**Time:** 1:45 to 5:30 p.m. (NZST)

**Instructor:** Toby Reynolds

**Course Fee:** NZ $225.00 (plus GST and ticketing fees)

**Registration Site:** [https://events.humanitix.com/owaspnz2026-training](https://events.humanitix.com/owaspnz2026-training){: target="training-reg" }

## Prerequisites

* Basic familiarity with the Linux command line is recommended but not required.

## What Students Should Bring and Do Before Class

* A Windows or MacOS laptop capable of running a VMware virtual machine with 4GB of memory. (A VMware installer will be provided with the course materials, if needed.)

## What the Students Will Be Provided With

* A virtual machine to use for the training.
* The slides.
* A lab pack.
* The testing environment.

I also plan to add each attendee to my online training course, so they can continue their learning with this, as I cover even more material online.

## Course Description

Embark on a journey into the heart of WiFi technology with our dynamic training program. Delve into the core principles while keeping pace with the latest advancements in the field. This immersive experience isn’t just about theory; it’s about hands-on learning. Navigate through virtual wireless arenas, applying newfound skills in real-world exercises.

From tackling personal networks to infiltrating enterprise setups, this training equips you to handle diverse challenges. Explore both fortified and vulnerable configurations, honing your expertise in thwarting attacks. And with a focus on the cutting-edge WPA3 standard, you’ll be prepared for the newest frontiers of WiFi security.

Each attendee will be provided a VMware Virtual Machine to use during the training and will contain everything needed to both learn and practice everything taught in this hands-on training event.

The course will cover the following topics:

* IEEE standards related to WiFi
* Deep dive into the wireless frames / packets (management frames, the state machine)
* Adapters and antenna types and use cases
* Common tooling
* Live demos and exercises breaking the WPA Handshake
* Deep dive into enterprise setups (EAP-PEAP, EAP-TLS)
* Live demo and exercises in breaking enterprise WiFi
* Introduction to WPA3 - useful attacks against it as well as a live demo and exercises
* Deep dive into the PMKID (used with WPA2) live demo and breaking
* And a module in wireless pivots and industry techniques for breaking into enterprise networks.

## Your Instructor

<img src="/assets/images/2026/Photo-Toby_Reynolds.jpg" alt="Photo of Toby Reynolds" style="float: left; width: 125px; margin-right: 15px;" />

**Toby “TheXero” Reynolds** is a dynamic security professional, with over a decade of experience, His career spans a diverse clientele in both commercial and non-commercial sectors. With a keen focus on enhancing cybersecurity, his expertise lies in vulnerability research, exploit development, and blackbox Penetration Testing. As a thought leader in the field, Toby not only identifies and addresses security gaps but also takes the lead as the primary trainer in courses that delve into the intricacies of attacker tools and methodologies. By combining practical experience with a passion for education, he empowers others to navigate the ever-evolving landscape of cybersecurity with confidence.
